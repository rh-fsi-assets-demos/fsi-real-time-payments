	...
	public void configure() throws Exception {
		...
		from("kafka:" + kafkaConsumerTopic + "?brokers=" + kafkaBootstrap + "&maxPollRecords=" + consumerMaxPollRecords
				...
				+ "&valueDeserializer=" + rtp.demo.domain.payment.serde.PaymentDeserializer.class.getName())
						.routeId("FromKafkaDebtorCreditor").log("\n/// Mock RTP Debtor -> Creditor >>> ${body}")
						.to("kafka:" + "mock-rtp-creditor-incoming" + "?serializerClass="
								+ rtp.demo.domain.payment.serde.PaymentSerializer.class.getName());

		from("kafka:" + "mock-rtp-creditor-acknowledgement" + "?brokers=" + kafkaBootstrap + "&maxPollRecords="
				+ consumerMaxPollRecords + "&consumersCount=" + consumerCount + "&seekTo=" + consumerSeekTo
				+ "&groupId=" + consumerGroup + "&valueDeserializer="
				+ rtp.demo.domain.payment.serde.PaymentDeserializer.class.getName()).routeId("FromKafkaConfimation")
						.log("\n/// Mock RTP Confirmation >>> ${body}")
						.to("kafka:" + "mock-rtp-debtor-confirmation" + "?serializerClass="
								+ rtp.demo.domain.payment.serde.PaymentSerializer.class.getName())
						.to("kafka:" + "mock-rtp-creditor-confirmation" + "?serializerClass="
								+ rtp.demo.domain.payment.serde.PaymentSerializer.class.getName());
		;
